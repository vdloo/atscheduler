...
def format_at_commands(commands, at):
    return list(map(lambda command: "echo {} 2>&1 | at {}".format(dumps(command), at), commands))
...
def create_at_generator(at, interval):
    ...
    def at_generator(_dt, _interval):
        while True:
            yield _dt.strftime('%H:%M %d %b %Y')
            _dt = _dt + timedelta(minutes=_interval)
